...
/**
 * Created by EduardoKarakawa on 13/03/2017.
 */
public class Atividade01{
    ...
    public BufferedImage triplicateimg(BufferedImage img){
        BufferedImage out = new BufferedImage(img.getWidth(), img.getHeight() * 3, BufferedImage.TYPE_INT_RGB);
        int aux = img.getHeight();
        for(int y = 0; y < img.getHeight(); y++){
            for(int x = 0; x < img.getWidth(); x++) {
                out.setRGB(x, y, img.getRGB(x, y));
                out.setRGB(x, y + aux, img.getRGB(x, y));
                out.setRGB(x, y + aux * 2, img.getRGB(x, y));
            }
        }
    ...
    public BufferedImage convert(BufferedImage img, int typepallete){
        //BufferedImage out = new BufferedImage(img.getWidth() * 2, img.getHeight(), BufferedImage.TYPE_INT_RGB);
        BufferedImage out = triplicateimg(img);
        ...
        for(int y = 0; y < img.getHeight(); y++){
            for(int x = 0; x < img.getWidth(); x++){
                ...
                out.setRGB(x , y + img.getHeight(), convertcolorto(originalpixel, typepallete));
    ...
    public void run() throws IOException {
        BufferedImage img = ImageIO.read(new File("C:\\Users\\B155 FIRE V3\\Downloads\\Image\\img\\cor\\turtle.jpg"));

        BufferedImage paleta64 = convert(img, 64);
        ImageIO.write(paleta64, "png", new File("paletea64img.png"));

        BufferedImage paleta48 = convert(img, 48);
        ImageIO.write(paleta48, "png", new File("paletea48img.png"));

        BufferedImage paleta16 = convert(img, 16);
        ImageIO.write(paleta16, "png", new File("paletea16img.png"));
